...
(async()=>{
  ...
  const batch=[{number:'0001',name:'架空 太郎',school:'',grade:'中1'},{number:'0002',name:'架空 花子',school:'架空学校',grade:'小6'}];
  const importBody={action:'import_students',students:batch,requestId:'batch-import'};
  assert.equal((await call(importBody,student)).status,403);
  const beforeImport=row.data.students.length;
  assert.equal((await call({...importBody,students:[batch[0],batch[0]],requestId:'batch-dupe'},admin)).status,409);
  assert.equal(row.data.students.length,beforeImport);
  assert.equal((await call({...importBody,students:[batch[0],{...batch[1],grade:'大学'}],requestId:'batch-invalid'},admin)).status,400);
  assert.equal(row.data.students.length,beforeImport);
  assert.equal((await call(importBody,admin)).status,200);
  assert.equal((await call(importBody,admin)).status,200);
  assert.equal(row.data.students.length,beforeImport+2);
  assert.equal(row.data.students.at(-2).number,'0001');
  assert.equal((await call({...importBody,requestId:'batch-existing'},admin)).status,409);
  const csvSource=fs.readFileSync(root+'/index.html','utf8');
  vm.runInContext(csvSource.slice(csvSource.indexOf('function parseStudentCSV('),csvSource.indexOf('function StudentCSVImport(')),ctx);
  ctx.csv='\ufeff生徒番号,生徒名,学校,学年\r\n0001,"架空,太郎","架空""学校",中1\r\n,,,\r\n';
  const parsed=vm.runInContext('parseStudentCSV(csv)',ctx);
  assert.equal(parsed.length,1);assert.equal(parsed[0].number,'0001');assert.equal(parsed[0].name,'架空,太郎');assert.equal(parsed[0].school,'架空"学校');
  ctx.importRows=parsed;ctx.existingStudents=[];
  assert.equal(vm.runInContext('studentImportErrors(importRows,existingStudents).length',ctx),0);
